...
void on_frame_ready(crdc::airi::H264DecoderData* decoder_data, uint8_t* frame_buf, int frame_size){
  static int count = 0;
...
int main(int argc, char* argv[]) {
  ...
  crdc::airi::decoder_set_frame_ready_handler(decoder_data, on_frame_ready);
  in_buffer = malloc(sizeof(uint8_t) * (IN_BUFFER_SIZE + crdc::airi::decoder_input_buffer_padding_size()));
  
  while(( cur_size = fread(in_buffer, 1, IN_BUFFER_SIZE, fp_in)) > 0){
    crdc::airi::decoder_parse(decoder_data, in_buffer, cur_size);
  }
  // flush parser
