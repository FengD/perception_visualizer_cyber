...
int main( int argc, char **argv ){
  ...
  crdc::airi::H264EncoderData* encoder_data;

  if(crdc::airi::encoder_init(&encoder_data, width, height, false) < 0){
  ...
  while(fread(raw_buffer, 1, raw_byte_size, fp_in) > 0){
    ...
    // encode it
    if(crdc::airi::encoder_encode(encoder_data,&out_buffer, &out_size) < 0){
      printf("Encode error\n");
      goto fail;
    }
